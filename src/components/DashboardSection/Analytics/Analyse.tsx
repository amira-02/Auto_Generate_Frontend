      ...
      <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #f0f0f0",
        ...
        <div style={{ display: "flex", background: "#f8f9fb", borderRadius: 10, padding: 3, border: "1px solid #f0f0f0" }}>
          {([
            ...
            const tabAccent = t.id === "facebook" ? "#1877f2" : t.id === "instagram" ? "#e1306c" : "#6366f1";
            return (
      ...
      {activeTab === "overview" && (
        <>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(190px, 1fr))", gap: 14, marginBottom: 20 }}>
            ...
            <KpiCard label="IG Eng. Rate"     value={`${engRate}%`}          accent="#8b5cf6" icon="💬" delay={0.3}       sub="likes+comments/followers" badge="REAL" badgeColor="#e1306c" />
            <KpiCard label="Scheduled"        value={scheduled.length}       accent="#8b5cf6" icon="📅" delay={0.35}      sub="upcoming" />
          </div>
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
              ...
              <ResponsiveContainer width="100%" height={200}>
                <AreaChart data={monthlyTrend} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                  <defs>
                    <linearGradient id="gT" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#6366f1" stopOpacity={0.15} /><stop offset="95%" stopColor="#6366f1" stopOpacity={0} /></linearGradient>
      ...
      {activeTab === "instagram" && (
        <>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(190px, 1fr))", gap: 14, marginBottom: 20 }}>
            ...
            <KpiCard label="Total Posts"    value={igMediaCount}            accent="#8b5cf6" icon="🖼️" delay={0.05} sub="published on IG"          badge="REAL" badgeColor="#e1306c" />
            ...
            <KpiCard label="Avg Likes/Post" value={igMediaCount > 0 ? Math.round(igTotalLikes / igMediaCount) : 0} accent="#6366f1" icon="⭐" delay={0.25} sub="per post" badge="REAL" badgeColor="#e1306c" />
          </div>
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
              ...
              <ResponsiveContainer width="100%" height={210}>
                <LineChart data={monthlyTrend} margin={{ top: 0, right: 10, left: -20, bottom: 0 }}>
                  <defs><linearGradient id="lG" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="#6366f1" /><stop offset="100%" stopColor="#8b5cf6" /></linearGradient></defs>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f5f5f5" />
                  <XAxis dataKey="month" tick={{ fontSize: 11, fill: "#94a3b8" }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fontSize: 11, fill: "#94a3b8" }} axisLine={false} tickLine={false} />
                  <Tooltip content={<CustomTooltip />} />
