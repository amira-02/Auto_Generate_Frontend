...
export default function CreatePostModal(props: Props) {
  ...
  useEffect(() => {
    if (modal.open) {
      ...
      setM({ generatedImage: "", success: "", error: "", postId: 0, uploadedImages: [], uploadedVideo: null });
      setLocalImages([]);
      setLocalVideo(null);
...
- Plateformes : ${modal.selectedPlatforms.join(", ")}`;
  ...
  // ── Internal upload handlers (override parent props) ─────────────────────────

  const handleImageUploadInternal = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = Array.from(e.target.files ?? []);
    if (files.length === 0) return;

    // Upload all files in parallel, collect URLs
    ...
    // Single state update with ALL new URLs at once — use local state to avoid stale merge issue
    if (succeeded.length > 0) {
      setLocalImages(prev => [...prev, ...succeeded]);
    }

    if (failed > 0) {
      setM({ error: `❌ ${failed} file(s) failed to upload` });
    }
  ...
  const handleVideoUploadInternal = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setM({ error: `❌ Video upload failed: ${err.message}` });
    }
    if (videoInputRef.current) videoInputRef.current.value = "";
  };

  // ── Image Generation ─────────────────────────────────────────────────────────
  // Always uploads to Cloudinary via /api/images/upload-temp
  // Real URL stored in uploadedImages, sent during save
  ...
  const savePostWithStatus = async (status: PostStatus) => {
    ...
    try {
      ...
      // Use local state for images/video (avoids stale modal state issue)
      ...
      const payload: any = {
        ...
        hashtags:      modal.hashtags,
        status,
        imageUrl:      allImages[0] ?? "",         // main image (backward compat)
        imageUrls:     allImages,                   // ✅ all images
        videoUrl:      videoUrl ?? null,             // ✅ video
      };
          ...
          <div style={{
              ...
              {localImages.length > 1 && (
                <div style={{ marginTop: 12 }}>
                  ...
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    {localImages.slice(1).map((img: string, i: number) => (
                      <div key={i} style={{ position: "relative", width: 60, height: 60 }}>
                        <img src={img} style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: 8 }} alt="" />
                        <button
                          onClick={() => setLocalImages(prev => prev.filter((_, idx) => idx !== i + 1))}
            ...
            <div style={{ background: "#fff", borderRadius: 16, padding: 16, boxShadow: "0 1px 2px rgba(0,0,0,0.05)" }}>
              ...
              <div style={{ fontSize: 11, color: "#94a3b8", marginBottom: 14 }}>
                Choisis un statut puis clique sur Save
              </div>

              {/* Status selector */}
              {[
                { value: "Draft",    icon: "📄", label: "Draft",     sub: "Brouillon, pas de planification",     color: "#6b7280", bg: "#f9fafb", border: "#e5e7eb", active: "#f1f5f9", activeBorder: "#94a3b8" },
                { value: "InReview", icon: "🔍", label: "In Review", sub: "Nécessite validation",                color: "#d97706", bg: "#fffbeb", border: "#fde68a", active: "#fffbeb", activeBorder: "#f59e0b" },
                { value: "Approved", icon: "✅", label: "Approved",  sub: "Prêt à publier ou planifier",         color: "#059669", bg: "#f0fdf4", border: "#a7f3d0", active: "#f0fdf4", activeBorder: "#10b981" },
              ].map(opt => {
                const isSelected = postStatus === opt.value;
                const disabled = opt.value !== "Draft" && !confirmedCaption;
                return (
                  <button key={opt.value}
                    onClick={() => !disabled && setPostStatus(opt.value as PostStatus)}
                    disabled={disabled}
                    style={{
                      width: "100%", padding: "10px 14px", borderRadius: 10, marginBottom: 8,
                      background: isSelected ? opt.active : "#fff",
                      color: disabled ? "#9ca3af" : opt.color,
                      fontWeight: 600,
                      cursor: disabled ? "not-allowed" : "pointer",
                      textAlign: "left", display: "flex", alignItems: "center", gap: 10,
                      border: `1.5px solid ${isSelected ? opt.activeBorder : "#f0f0f0"}`,
                      transition: "all .15s",
              ...
              {/* Planification — visible si Approved sélectionné */}
              {postStatus === "Approved" && confirmedCaption && (
                <div style={{ borderTop: "1px solid #f0f0f0", paddingTop: 14, marginBottom: 14 }}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <input type="date" value={modal.scheduleDate || ""}
                      onChange={e => setM({ scheduleDate: e.target.value })}
                      style={{ ...S.input, flex: 1 }} />
                    <input type="time" value={modal.scheduleTime || ""}
              ...
              <button
                onClick={() => {
                  const finalStatus: PostStatus =
                    postStatus === "Approved" && modal.scheduleDate && modal.scheduleTime
                      ? "Scheduled"
                      : postStatus;
                ...
                disabled={saving}
                style={{
                  width: "100%", padding: "13px", borderRadius: 11, border: "none", marginTop: 4,
                ...
                {saving ? "⏳ Saving..." : (
                  postStatus === "Approved" && modal.scheduleDate && modal.scheduleTime
                    ? `📅 Schedule for ${modal.scheduleDate} ${modal.scheduleTime}`
                    : postStatus === "Approved"
                    ? "🚀 Save & Approve"
                    : postStatus === "InReview"
                    ? "🔍 Save for Review"
