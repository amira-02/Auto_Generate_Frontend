      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ padding: "14px 24px", borderBottom: "1px solid #e5e7eb", display: "flex", justifyContent: "space-between", alignItems: "center", background: "#fafafa" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            {isExternalTask && (
              <span style={{ fontSize: 11, background: "#f0f4ff", color: "#4f46e5", padding: "2px 8px", borderRadius: 20, fontWeight: 600 }}>
                🔗 External Task
              </span>
            )}
            {modal.topicId && (
        ...
        <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
          ...
          <div style={{ width: 280, minWidth: 260, flexShrink: 0, background: "#f9fafb", padding: 16, display: "flex", flexDirection: "column", gap: 16, overflowY: "auto", scrollbarWidth: "none" }}>
            ...
            {isExternalTask && (
              <div style={{ background: "#f0f4ff", borderRadius: 12, padding: "12px 14px", border: "1px solid #c7d2fe" }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: "#4f46e5", marginBottom: 4 }}>🔗 External Post Request</div>
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  {(["short", "medium", "long"] as const).map(v => (
                    <button key={v} onClick={() => setM({ captionLength: v })} style={{ flex: 1, padding: "6px", borderRadius: 10, border: "1px solid", borderColor: modal.captionLength === v ? "#3b82f6" : "#e5e7eb", background: modal.captionLength === v ? "#eff6ff" : "#fff", color: modal.captionLength === v ? "#3b82f6" : "#6b7280", fontSize: 11, cursor: "pointer" }}>
              ...
              <div>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {(["professional", "casual", "funny", "inspirational"] as const).map(v => (
                    <button key={v} onClick={() => setM({ tone: v })} style={{ padding: "6px 12px", borderRadius: 20, border: "1px solid", borderColor: modal.tone === v ? "#3b82f6" : "#e5e7eb", background: modal.tone === v ? "#eff6ff" : "#fff", color: modal.tone === v ? "#3b82f6" : "#6b7280", fontSize: 11, cursor: "pointer" }}>
          ...
          <div style={{ flex: 1, background: "#fff", display: "flex", flexDirection: "column", overflow: "hidden", borderLeft: "1px solid #e5e7eb", borderRight: "1px solid #e5e7eb" }}>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "20px", display: "flex", flexDirection: "column", gap: 16, scrollbarWidth: "none" }}>
              ...
              {isExternalTask && confirmedCaption && messages.length === 0 && (
                <div style={{ background: "#f0f4ff", borderRadius: 16, padding: "16px 20px", border: "1px solid #c7d2fe" }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "#4f46e5", marginBottom: 8 }}>📋 Caption from external request</div>
              ...
              {messages.map(msg => (
                <div key={msg.id} style={{ display: "flex", flexDirection: "column", alignItems: msg.role === "user" ? "flex-end" : "flex-start", width: "100%" }}>
                  <div style={{ maxWidth: "85%", padding: "12px 16px", borderRadius: msg.role === "user" ? "20px 20px 4px 20px" : "20px 20px 20px 4px", background: msg.role === "user" ? "#3b82f6" : "#fff", color: msg.role === "user" ? "#fff" : "#111827", fontSize: 13, lineHeight: 1.55, boxShadow: "0 1px 2px rgba(0,0,0,0.05)", border: msg.role === "bot" ? "1px solid #e5e7eb" : "none" }}>
                    {msg.content}
                  </div>
                      ...
                      {Object.keys(msg.captions).length > 1 && (
                        <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
                          {Object.keys(msg.captions).map(platform => (
                            <button key={platform} onClick={() => setActivePlatformTab(platform)} style={{ padding: "6px 14px", borderRadius: 20, border: "1px solid", borderColor: activePlatformTab === platform ? "#3b82f6" : "#e5e7eb", background: activePlatformTab === platform ? "#eff6ff" : "#fff", color: activePlatformTab === platform ? "#3b82f6" : "#6b7280", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>
                              {platform}
                            </button>
                          ))}
                        </div>
            ...
            <div style={{ padding: "16px 20px", background: "#fff", borderTop: "1px solid #e5e7eb" }}>
              <div style={{ display: "flex", gap: 12 }}>
                ...
                <button onClick={handleUserSend} disabled={chatLoading || !userInput.trim()}
                  style={{ padding: "10px 20px", background: (!userInput.trim() || chatLoading) ? "#e5e7eb" : "#3b82f6", color: (!userInput.trim() || chatLoading) ? "#9ca3af" : "#fff", border: "none", borderRadius: 12, fontWeight: 700, cursor: "pointer" }}>
                  Envoyer
                </button>
              </div>
            </div>
          ...
          <div style={{ width: 340, minWidth: 300, flexShrink: 0, background: "#f9fafb", padding: 16, display: "flex", flexDirection: "column", gap: 16, overflowY: "auto", scrollbarWidth: "none" }}>
            ...
            <div style={{ background: "#fff", borderRadius: 16, padding: 16, boxShadow: "0 1px 2px rgba(0,0,0,0.05)" }}>
              ...
              <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
                {["realistic", "artistic", "cartoon", "minimalist"].map(style => (
                  <button key={style} onClick={() => setImageStyle(style)} style={{ flex: 1, padding: "4px", borderRadius: 20, border: "1px solid", borderColor: imageStyle === style ? "#3b82f6" : "#e5e7eb", background: imageStyle === style ? "#eff6ff" : "#fff", color: imageStyle === style ? "#3b82f6" : "#6b7280", fontSize: 10, cursor: "pointer" }}>{style}</button>
                ))}
              </div>
              <button onClick={generateImage} disabled={generatingImage || !imagePrompt.trim()} style={{ width: "100%", padding: "10px", borderRadius: 10, border: "none", background: (!imagePrompt.trim() || generatingImage) ? "#e5e7eb" : "#3b82f6", color: (!imagePrompt.trim() || generatingImage) ? "#9ca3af" : "#fff", fontWeight: 600, cursor: "pointer" }}>
            ...
            <div style={{ background: "#fff", borderRadius: 16, padding: 16, boxShadow: "0 1px 2px rgba(0,0,0,0.05)" }}>
              ...
              <button
                ...
                disabled={saving}
                style={{ width: "100%", padding: "13px", borderRadius: 11, border: "none", marginTop: 4, background: saving ? "#e5e7eb" : "linear-gradient(135deg, #6366f1, #8b5cf6)", color: saving ? "#9ca3af" : "#fff", fontWeight: 700, fontSize: 14, cursor: saving ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, boxShadow: saving ? "none" : "0 4px 14px rgba(99,102,241,0.3)", transition: "all .15s" }}>
                {saving ? "⏳ Saving..." : (
