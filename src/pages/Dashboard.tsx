...
type ModalState = {
  ...
  scheduleDate?: string; scheduleTime?: string; success?: string;
  topicId?: number;
};
...
function StatCard({ label, value, accent }: { label: string; value: number | string; accent: string }) {
  return (
    ...
    >
      <span style={{ fontSize: 28, fontWeight: 700, color: "#0f172a", fontFamily: "'DM Mono', monospace", letterSpacing: "-1px" }}>
        {value}
      </span>
  ...
  return (
    <motion.button
      ...
      style={{
        ...
        border: `1px solid ${active ? "#e0e0fd" : "#f0f0f0"}`,
        background: active ? "#f0f0fe" : "#fff",
        cursor: "pointer",
...
export default function Dashboard() {
  ...
  return (
    <div style={{
      ...
      background: "#f8f9fb", fontFamily: "'DM Sans', system-ui, sans-serif",
    }}>
      ...
      >
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 2 }}>
          {sidebarOpen && (
            <p style={{ fontSize: 10, fontWeight: 600, color: "#cbd5e1", letterSpacing: "0.08em", padding: "0 4px 8px", textTransform: "uppercase", margin: 0 }}>
              Workspace
            </p>
          )}
          {NAV.map(item => {
            ...
            return (
              <button key={item.id}
                onClick={() => { setActiveNav(item.id); if (item.id !== "topics") setSelectedTopic(null); }}
        ...
        <div style={{ borderTop: "1px solid #f5f5f5", paddingTop: 14, display: "flex", flexDirection: "column", gap: 8, alignItems: sidebarOpen ? "stretch" : "center" }}>
          ...
          {sidebarOpen && (
            <button
              ...
              style={{
                padding: "7px 12px", borderRadius: 10, border: "1px solid #f0f0f0",
              ...
              <motion.div key="dashboard" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                ...
                <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #f0f0f0", overflow: "hidden" }}>
                  <div style={{ padding: "16px 20px", borderBottom: "1px solid #f5f5f5", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <button onClick={() => setActiveNav("posts")} style={{ fontSize: 12, color: "#6366f1", background: "none", border: "none", cursor: "pointer", fontWeight: 500 }}>
                      View all →
                    </button>
                  </div>
                  {loadingPosts ? (
                    ...
                    posts.slice(0, 6).map((post, i) => {
                      const statusColors: Record<string, string> = {
                        published: "#10b981", scheduled: "#f59e0b",
                        draft: "#94a3b8", failed: "#ef4444", inreview: "#3b82f6",
                      };
                      const color = statusColors[post.status?.toLowerCase()] ?? "#94a3b8";
                      return (
                        <div key={post.id}
                          style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 20px", borderBottom: i < 5 ? "1px solid #f9f9f9" : "none", transition: "background .1s" }}
                        ...
                        >
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <p style={{ margin: 0, fontSize: 13, color: "#374151", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", fontWeight: 500 }}>
                              {post.caption ?? post.topicName ?? "Untitled"}
                            </p>
                            <p style={{ margin: 0, fontSize: 11, color: "#94a3b8", marginTop: 2 }}>
                              {post.topicName} · {new Date(post.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                            </p>
                          </div>
                          <div style={{ padding: "3px 10px", borderRadius: 20, background: color + "15", color, fontSize: 11, fontWeight: 600, flexShrink: 0, textTransform: "capitalize" }}>
                            {post.status}
                          </div>
                        </div>
                      );
